...
//    var cropSlidervw: OptiRangeSliderView!
    ...
    override func viewDidLoad() {
        ...
        self.setTitle("Post A Video", andImage: UIImage(named: "logo")!)
        
        if videoCategory == "stories" {
            
            self.setTitle("Post A Story", andImage: UIImage(named: "logo")!)
        }
        
        let backBtn = UIBarButtonItem(title: "Back", style: .done, target: self, action: #selector(self.backF))
        ...
        DispatchQueue.main.async {
            ...
            alert.addAction(UIAlertAction(title: "Continue", style: UIAlertAction.Style.default, handler: { action in
                if let videourl = self.slctVideoUrl {
                    ...
                    let playerViewController = PublishVideoViewController()
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let videourl = videoURL {
            if isMergeClicked == true {
                ...
                if self.videoTotalsec > 6 {
                    
                    OptiVideoEditor().trimVideo(sourceURL: videourl, startTime: 0, endTime: 6, success: { (url) in
                        DispatchQueue.main.async {
                            let saveBarBtnItm = UIBarButtonItem(title: "Save", style: .done, target: self, action: #selector(self.saveActionforEditedVideo))
                            self.navigationItem.rightBarButtonItem  = saveBarBtnItm
                            self.progress_Vw.progress = 1.0
...
extension OptiViewController : UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case 1:
            ...
            cell.imgvw_Menu.image = UIImage(named:menuItems[indexPath.row])?.withRenderingMode(.alwaysTemplate)
            
            cell.imgvw_Menu.tintColor = .white
            return cell
        case 2:
